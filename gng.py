...
class GrowingNeuralGas:
    ...
    def find_nearest_units(self, observation, k=2):
        distance = []
        for unit in self.units:
            distance.append(spatial.distance.euclidean(unit, observation))
        ranking = np.argsort(distance)
        return ranking[-k:].reverse()
    ...
    def train(self, e_b, e_n, a_max, l, a, d):
        # start with two units a and b at random position w_a and w_b
        w_a = np.random.uniform(-1, 1, [len(self.data[0]), 1])
        w_b = np.random.uniform(-1, 1, [len(self.data[0]), 1])
...
if __name__ == '__main__':
    ...
    # gng.train(e_b=0.2, e_n=0.006, a_max=50, l=100, a=0.5, d=0.995)
